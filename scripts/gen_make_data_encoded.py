for n in [50000]:
    ...
    #  ["sonic_hilltop_100000_pretrained","HillTopZone.Act1/quamassia-cervus-plateful","jlbaker361/sonic-vae2-HillTopZone.Act1"],
        [f"sonic_metro_{n}_encoded","MetropolisZone.Act11/uzbekistan-goldilocks-solicitor"],
        [f"sonic_casino_{n}_encoded","CasinoNightZone.Act11/complexness-kanarese-oleander"],
        [f"sonic_aqua_{n}_encoded","AquaticRuinZone.Act11/cetrimide-bovidae-schooldays"],
        [f"sonic_chemical_{n}_encoded","ChemicalPlantZone.Act11/bowdlerisation-mishegoss-uncial"],
        [f"sonic_hilltop_{n}_encoded","HillTopZone.Act11/acanthocytosis-gasterophilus-patrioteer"],
        [f"sonic_emerald_{n}_encoded","EmeraldHillZone.Act11/constrictor-lorazepam-sphaerocarpales"]
    ]:
        scenario=path.split("/")[0]
        ...
        command+=f" --upload_path jlbaker361/{name} --folder {base_path}{path}  --vae_checkpoint jlbaker361/sonic-vae50000-{scenario} "
        print(command)
