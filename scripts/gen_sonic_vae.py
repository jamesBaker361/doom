...
for n in [50000]:
   ...
   for gpus in [2]:
      for zone in ["MetropolisZone.Act11/uzbekistan-goldilocks-solicitor",
         ...
         command=f"sbatch -J vae  --nodelist=g20-02,g20-03,g20-04,g20-05,g20-09,g20-12 --err=slurm_chip/vae/{n}_{zone_name}/sonic.err --out=slurm_chip/vae/{n}_{zone_name}/sonic.out "
         command+=" runpygpu_chip.sh "
         port+=1
         command+=f" vae.py --image_folder_paths "
         command+=f" {base_path}{zone} "
         command+=f" --batch_size 2 --gradient_accumulation_steps 16  --limit -1 --epochs 100 --image_interval 5 --name jlbaker361/sonic-vae{n}-{zone_name} "
