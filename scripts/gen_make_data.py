...
for [name,path,pretrained] in [
    ...
    ["sonic_metro_100000_pretrained","MetropolisZone.Act1/sonograph-tinner-laudability","jlbaker361/sonic-vae2-MetropolisZone.Act1"]
]:
    command=f"sbatch -J data --err=slurm_chip/data/{name}.err --out=slurm_chip/data/{name}.out runpygpu_chip.sh make_dataset.py "
    command+=f" --upload_path jlbaker361/{name} --folder {base_path}{path} --vae_checkpoint {pretrained} "
    print(command)